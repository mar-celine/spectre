...
Domain<3> Shell::create_domain() const {
  /*  const domain::CoordinateMaps::EquatorialCompression compression{
    ...
    domain::make_vector_coordinate_map_base<Frame::BlockLogical,
                                                              Frame::Inertial,
    3>( sph_wedge_coordinate_maps( inner_radius_, outer_radius_, 1.0, 1.0,
    use_equiangular_map_, false, radial_partitioning_, radial_distribution_,
    which_wedges_), compression);
  */
    ...
    if (shape_.has_value()) {
      ...
      /*      const DataVector radial_distortion =
                1. - get(gr::Solutions::kerr_horizon_radius(
                         ...
                         inner_radius_;*/
      //      auto radial_distortion_coefs =
      //      ylm.phys_to_spec(radial_distortion);
      ...
      /* {{0., 0., 0.}},
         ...
         std::make_unique<domain::CoordinateMaps::ShapeMapTransitionFunctions::
                                  SphereTransition>(inner_radius_,
         outer_radius_), std::move(f_of_t_nameradial_distortion_coefs)};*/

      // set function of time to take the ylm coeffs
      // the shape map doesnt take the ylm coeffs it takes the functions of time
